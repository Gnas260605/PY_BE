...
def bottom_nav(role: str) -> None:
    with ui.footer().classes("md:hidden bg-white border-t border-slate-200 text-slate-700 z-30"):
        with ui.row().classes("w-full justify-around"):
            for item in NAV_ITEMS.get(role, NAV_ITEMS["USER"])[:4]:
                label, target, icon = item[0], item[1], item[2]
